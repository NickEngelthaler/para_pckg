...
from Detect_Wordbased_TPL.separate_str import separate_str
from Detect_Wordbased_TPL.check_tempo import check_tempo
from Detect_Wordbased_TPL.check_emphasis import check_emphasis
from Detect_Wordbased_TPL.check_pitch import check_pitch
from Detect_Wordbased_TPL.check_stress import check_stress
#import enchant
import re 
def Detect_Wordbased_TPL(textline, keyword_dic,acron_list, brown_dic):
    ...
    for phrase in phrase_list: # match phrase kw in the new text one after another using regular expression
        ...
        if len(phrase_temp)>2 or len(phrase_temp) is 2:
    ...
    for w in wordlist:
        if w != "tplandrea" and (w in en_case or w in tpl_case):
            ...
            if len(pitch_temp)>0 and last_w is 0:
                pitch_case = pitch_case + [w]
            elif w.isupper() and len(w) is 1 and last_w is not 0:
                pitch_temp = ["temp"]
        elif (w.isupper() and last_w is not 0) or w in ["&",".",",","?","!",":",";","$","(",")","[","]","|","{","}","<",">"]:
            pitch_temp = ["temp"]
        else:
            pitch_temp = []
    ...
    for w in wordlist:
        if w != "tplandrea" and (w in en_case or w in tpl_case):
            ...
            if len(stress_temp)>0 and last_w is 0: # last_w is to avoid duplicated counting stress
                stress_case = stress_case + [w]
            elif w.isupper() and last_w is not 0:
                stress_temp = ["temp"]
        elif (w.isupper() and last_w is not 0) or w in ["&",".",",","?","!",":",";","$","(",")","[","]","|","{","}","<",">"]:
            stress_temp = ["temp"]
        else:
